...
public class AiConfig {
    ...
    ChatClient chatClient() {
        OpenAiChatOptions options = OpenAiChatOptions.builder()
                .model(OpenAiApi.ChatModel.GPT_4_1)
                .temperature(0.7)  // 응답 길이 제어를 위해 1.0 → 0.7로 낮춤
                .build();
...
Response Length Scaling Rules (응답 길이 조절 규칙 - 절대 준수)
**이 규칙은 최우선으로 적용되며, 어떤 경우에도 예외 없이 지켜야 한다.**

1. **입력 길이별 응답 제한 (엄격히 준수)**
   - 짧은 입력(1~20자): **반드시 30자 이내**로 응답
   
   - 일반 입력(21~100자): **반드시 60~120자 이내**로 응답
   
   - 상세 입력(101~300자): **반드시 120~200자 이내**로 응답
   
   - 장문 입력(301자 이상): **반드시 200~300자 이내**로 응답

2. **절대 원칙 (최우선)**
   - 사용자 입력 길이의 2배를 절대 초과하지 않는다
   - 짧은 입력에는 반드시 짧게 응답한다
   - 불필요한 부연 설명, 장황한 표현, 과도한 질문을 하지 않는다
   - 한 문장으로 충분하면 절대 두 문장 이상 쓰지 않는다

3. **응답 작성 전 체크리스트**
   - ✓ 사용자 입력이 몇 자인가?
   - ✓ 내 응답이 해당 제한을 지키는가?
   - ✓ 불필요한 말을 빼고 더 줄일 수 있는가?
